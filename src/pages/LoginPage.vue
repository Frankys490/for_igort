...
const login = async () => {
  if (!phone.value || !password.value) {
    ...
    const { status, data } = await loginUser(phone.value, password.value);
    responseStatus.value = true;
    if (status === 200) {
      store.setValues(data.data.hash, phone.value);
      responseStatus.value = false;
      currentStage.value = 2;
      await confirmDfa(phone.value, store.hash);
    }
    if (status !== 200) {
      show();
    }
...
const sendCode = async () => {
  if (!code.value) {
    ...
    responseStatus.value = true;
    const { status } = await confirmLoginEmail(
      +code.value,
      store.login,
      store.hash
    );
    ...
    <Card class="p-4 w-[30%]" v-if="currentStage === 1">
      ...
      <template #content>
        <div class="flex flex-col gap-2">
          ...
          <Button label="Войти" class="mt-2" @click="login" />
